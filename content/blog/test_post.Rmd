...
![](/blog/test_post_files/thetruth1_screenshot1.png){width=600px height=400px}

In this case it's pretty easy, the html nodes I'm interested in are simply "table" class. 

![](/blog/test_post_files/thetruth1_screenshot2.png){width=600px height=400px}

At first glance, it looks like tables 2 through 14 are "wiki episode table". That's one more than I would expect, because there are nine original seasons, and then two follow-up seasons that came out more recently (10-11). However, there are also two X-Files movies, which appear to have separate tables. I don't want to deal with those right now really, so I will leave them out when I scrape the tables. 
...
table_df <- table_df %>% 
  mutate(season = factor(season, levels = c("Season 1", "Season 2", "Season 3", "Season 4", 
...
p <- table_df %>% 
  ...
  geom_point_interactive(aes(tooltip = full_title, data_id = no_overall), size = 3, alpha = .75) +
...
girafe(ggobj = p,
  ...
  options = list(opts_tooltip(css = tooltip.css, 
                              use_fill = TRUE),
                 opts_sizing(width = .9) ) )
```
...
In the next episode I will be webscraping episode scripts to use in text analysis.
